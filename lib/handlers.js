...
function unknown (msg) {
  slack.send({
    icon: ':interrobang:',
    text: 'Unrecognized SNS message ```' + msg.payload + '```',
    channel: msg.channel,
  })
  msg.response.send('ok')
  return true
...
export function parseAsLineDelimitedKVPairs(message) {
  let kvRegex = /^(\w+)='?(.*?)'?$/
  return R.trim(message).split('\n').reduce((obj, line) => {
    let match = kvRegex.exec(line)
    if (!match) { throw new Error('Line does not match key-value format') }
